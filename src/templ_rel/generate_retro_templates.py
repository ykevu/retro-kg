"""
Source: https://github.com/connorcoley/retrosim/blob/master/retrosim/utils/generate_retro_templates.py
...
def expand_atoms_to_use(
    mol, atoms_to_use, groups=[], symbol_replacements=[], super_general=False
):
                    ...
                    symbol_replacements.append(
                        (
                            ...
                            convert_atom_to_wildcard(
                                neighbor, super_general=super_general
                            ),
                        )
                    )
...
def expand_atoms_to_use_atom(
    mol, atoms_to_use, atom_idx, groups=[], symbol_replacements=[]
):
    """Given an RDKit molecule and a list of AtomIdx which should be included
            ...
            if ":" in atom.GetSmarts():
                if atom.GetSmarts().split(":")[1][:-1] in changed_atom_tags:
                    ...
                    symbol = get_strict_smarts_for_atom(
                        atom, super_general=super_general
                    )
                    if symbol != atom.GetSmarts():
...
def process_an_example(reaction_smiles, super_general=False, v=False):
    ...
    try:
        ...
        for product in products:
            if sum([a.HasProp("molAtomMapNumber") for a in product.GetAtoms()]) < len(
                product.GetAtoms()
            ):
        ...
        if are_unmapped_product_atoms:  # add fragment to template
            ...
            if extra_reactant_fragment:
                ...
                if v:
                    print(
                        "    extra reactant fragment: {}".format(
                            extra_reactant_fragment
                        )
                    )
        ...
        try:
            ...
            reactant_fragments, intra_only = get_fragments_for_changed_atoms(
                reactants,
                changed_atom_tags,
                radius=1,
                expansion=[],
            ...
            product_fragments, _ = get_fragments_for_changed_atoms(
                products,
                changed_atom_tags,
                radius=0,
                expansion=expand_changed_atom_tags(
                    changed_atom_tags, reactant_fragments
                ),
                category="products",
                super_general=super_general,
            )
